...
class PhotoItem extends React.Component {
  ...
  renderAddToAlbum = () => {
    if (this.state.isAddedToAlbum) {
      return <button className="add-button default-button" onClick={() => this.removeFromAlbum()}><i className="fa fa-folder" aria-hidden="true"></i> Added to album</button>;
    }
    return <button className="add-button default-button" onClick={() => this.addToAlbum()}><i className="fa fa-folder-o" aria-hidden="true"></i> Add to album</button>;
  }
